...
- `[PI-1]` Refines umbrella `[PI-3]`: `src/render/` receives a path string and a source string, never a
...
- `[C-3]` Worktree markdown is trusted input: HTML in a document reaches the page unsanitised.
...
The endpoint accepts a worktree-relative path under `.plan/` with a `.md` suffix, or any
`*.spec.md(x)`, `*.discovery.md(x)`, `*.examples.md(x)` inside the worktree (umbrella `[KD-8]`), and
resolves both the path and its realpath against the worktree root per umbrella `[PI-3]`. Responses:
403 outside the worktree or for a disallowed suffix, 404 for a missing file, otherwise 200. A render
