...
plan-viewer browses the design corpus of every git worktree in a workspace: implementation plans under
`.plan/` and specs colocated with the code they describe. That corpus is authored in a richer dialect
...
- `[G-3]` Present plans and specs as a collapsible folder tree that also reflects `parent-spec`
  umbrella nesting.
- `[G-4]` Stay repo-agnostic: a worktree needs no renderer, config file, or build step of its own for
...
- `[PI-2]` Never blank — a readable file always produces a page, with degradation visible where it
  occurs.
- `[PI-3]` Confined reads — every path is resolved against the selected worktree and refused outside
  it, symlinks included; rendering widens nothing.
...
- `[NG-4]` No consumer configuration: no per-repo config file, corpus warnings, or feature
  dashboards; the viewer's behaviour is the same in every worktree.
...
```text
worktree files ──► corpus scan (server) ──► tree model ──► sidebar tree (client)
       │                                       ▲
...
| Doc endpoint    | Bun server route  | Serve rendered HTML plus metadata for one worktree-confined path             | `GET /api/doc`                                       |
...
Detailed design lives in the leaves; the umbrella owns `[KD-1]`–`[KD-8]` and `[PI-1]`–`[PI-5]`.
