...
This leaf owns umbrella `[G-3]` and the navigation half of `[G-5]`: getting a reader from a selected
worktree to one of a few hundred documents. Those documents carry two overlapping structures — the
...
| `[KD-3.1]` Umbrella owns a leaf folder row               | A directory holding no subdirectory and one spec with `kind: umbrella` renders as that spec's row, labelled with its title; a directory with subdirectories stays a folder and its umbrella is a row inside it      | One umbrella per leaf directory means the umbrella _is_ that folder's meaning, and two rows for one concept costs a click; a folder with subdirectories holds their specs too, so absorbing it would pull unrelated documents into the umbrella's family                                               |
...
| `[KD-6]` Expansion is derived state                      | Ancestors of the open document are expanded, the rest start collapsed, and toggles live in component state                                                                                                          | The URL already identifies project, worktree, tab and document, and re-deriving ancestors from that restores the useful part of the view, so persisting toggles adds state without adding recall                                                                                                       |
...
- `[PI-1]` Refines umbrella `[PI-3]`: discovery lists tracked files with `git ls-files` inside the
  worktree, so it cannot follow a link out of it.

## 5. Non-Goals

- `[NG-1]` Refines umbrella `[NG-1]`: the tree offers no drag, reorder, rename, or any other mutation.
- `[NG-2]` Refines umbrella `[NG-4]`: the tree is scoped to the selected worktree — no cross-worktree
  or cross-project view — and renders its rows directly, without virtualisation.
...
- `[C-3]` Expansion state is per session under `[KD-6]` and resets on reload.
...
3. **Nest by path** — split each worktree-relative path on `/` and insert directory nodes.
4. **Attach umbrellas** — in a directory with no subdirectory, the single document with
   `kind: umbrella` moves onto the directory node and lends it its title per `[KD-3.1]`; with two such
...
The sidebar holds the project and worktree selectors, the plans/specs tabs, and a recursive `DocTree`:
