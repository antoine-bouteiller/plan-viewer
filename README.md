...
otherwise the OS chooses an available port. Set `PLAN_VIEWER_BIND=<host>` to bind to another
address (e.g. the VM IP from `SSH_CONNECTION`); the server has no authentication, so only do this
